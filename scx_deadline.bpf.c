...
void print_priq()
{
	...
	struct task_ctx *q_tctx;
	bpf_printk("Priority queue [pid: abs dl]: ");
	bpf_for_each(scx_dsq, q, DEADLINE_PRIQ_DSQ, 0) {
		if (!(q_tctx = lookup_task_ctx(q)))
		{
			continue;
		}
		bpf_printk("[%d: %ull] ", q->pid, q_tctx->abs_deadline);
    }
	bpf_printk("\n");
}
*/

s32 BPF_STRUCT_OPS(deadline_select_cpu, struct task_struct *p, s32 prev_cpu, u64 wake_flags)
{
...
void BPF_STRUCT_OPS(deadline_enqueue, struct task_struct *p, u64 enq_flags)
{
...
void BPF_STRUCT_OPS(deadline_dequeue, struct task_struct *p, u64 deq_flags)
{
...
void BPF_STRUCT_OPS(deadline_dispatch, s32 cpu, struct task_struct *prev)
{
...
void BPF_STRUCT_OPS(deadline_running, struct task_struct *p)
{
...
void BPF_STRUCT_OPS(deadline_runnable, struct task_struct *p, u64 enq_flags)
{
...
void BPF_STRUCT_OPS(deadline_stopping, struct task_struct *p, bool runnable)
{
...
void BPF_STRUCT_OPS(deadline_enable, struct task_struct *p)
{
...
* 
*/
void BPF_STRUCT_OPS(deadline_cpu_release, s32 cpu, struct scx_cpu_release_args *args)
{
...
void BPF_STRUCT_OPS(deadline_cpu_acquire, s32 cpu, struct scx_cpu_acquire_args *args)
{
	bpf_printk("[DEBUG] [ACQUIRE] CPU %d is acquired\n", cpu);
	//scx_bpf_kick_cpu(cpu, SCX_KICK_PREEMPT);
}

void BPF_STRUCT_OPS(deadline_quiescent, struct task_struct *p, u64 deq_flags) {
	s32 cpu = scx_bpf_task_cpu(p);
...
s32 BPF_STRUCT_OPS_SLEEPABLE(deadline_init)
{
	bpf_printk("[INFO] [INIT] Starting SCX Deadline Scheduler\n");
	u32 zero = 0;
	struct running_task_ctx* cpu_i_running_task_ctx;
	s32 cpu;
	bpf_for(cpu, 0, scx_bpf_nr_cpu_ids()) {
...
void BPF_STRUCT_OPS(deadline_exit, struct scx_exit_info *ei)
{
	bpf_printk("[INFO] [EXIT] Exiting SCX Deadline Scheduler\n");
	UEI_RECORD(uei, ei);
}

s32 BPF_STRUCT_OPS(deadline_init_task, struct task_struct *p, struct scx_init_task_args *args)
{
...
void BPF_STRUCT_OPS(deadline_disable, struct task_struct *p)
{
	bpf_printk("[INFO] [DISABLE] Task %d (%s) disabled\n", p->pid, p->comm);
}

void BPF_STRUCT_OPS(deadline_dump, struct scx_dump_ctx *dctx)
{
...
void BPF_STRUCT_OPS(deadline_dump_task, struct scx_dump_ctx *dctx, struct task_struct *p)
{
	u64 dsq_id = 101;
	if (p->scx.dsq)
	{
...
SCX_OPS_DEFINE(deadline_ops,
	.select_cpu		= (void *)deadline_select_cpu,
	.enqueue		= (void *)deadline_enqueue,
	.dequeue		= (void *)deadline_dequeue,
	.dispatch		= (void *)deadline_dispatch,
	.runnable		= (void *)deadline_runnable,
	.running		= (void *)deadline_running,
	.stopping		= (void *)deadline_stopping,
	.quiescent		= (void *)deadline_quiescent,
	.cpu_acquire	= (void *)deadline_cpu_acquire,
	.cpu_release	= (void *)deadline_cpu_release,
	.enable			= (void *)deadline_enable,
	.disable		= (void *)deadline_disable,
	.init_task		= (void *)deadline_init_task,
	.exit_task		= (void *)deadline_exit_task,
	.init			= (void *)deadline_init,
	.exit			= (void *)deadline_exit,
	.dump			= (void *)deadline_dump,
	.dump_task		= (void *)deadline_dump_task,
